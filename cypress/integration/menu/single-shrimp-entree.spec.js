...
describe('Click single -- shrimp entree', () => {
    ...
    it('Check choices for shrimp entree', () => {
        const sql = 'select * from singles where name = "Shrimp Entree"'
        cy.task('queryDb', sql)
            .then((resultSingles) => {
                const id = resultSingles[0].id
                cy.get('#singleItem' + id).click()
                cy.wait(1000)
                const sql = 'select * from entrees where category = "Shrimp"'
                cy.task('queryDb', sql)
                    .then((resultEntrees) => {
                        for (var i=0; i<resultEntrees.length; i++) {
                            ...
                            const sql = 'select * from products where category = ' + '"Shrimp"'
                            cy.task('queryDb', sql)
                                .then((resultProducts) => {
