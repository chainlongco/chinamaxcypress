...
describe('Visit cart, add fried dumpling, and check in the cart', () => {
    it('Click appetizers, click plus button for fried dumpling, then check all information on the screen', () => {
        cy.get('#cart_count').contains('1')

        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
                // Image
    ...
    it('Click appetizers, click plus button for fried dumpling, then click plus button one more time', () => {
        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
    ...
    it('Click appetizers, click plus button for fried dumpling, then click plus button one time, then click minus button one time', () => {
        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
                // 1 is serialNumber in cart
    ...
    it('Click appetizers, click plus button for fried dumpling, then click Remove button', () => {
        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
                // cart count should be 1
    ...
    it('Click appetizers, click plus button for fried dumpling, then click Edit button -- change quantity to 3, 2, then click Update button', () => {
        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
                // cart count should be 1
    ...
    it('Click appetizers, click plus button for fried dumpling, then click Add More Items to same item', () => {
    ...
    it('Click appetizers, click plus button for fried dumpling, then click Add More Items to click plus button for egg roll', () => {
        cy.get('.addMoreItems').click()
        cy.wait(1000)
        cy.clickAppetizers()
        cy.wait(1000)
        cy.clickAppetizersPlus('Egg Roll (5)')
        cy.clickAppetizersAddToCart('Egg Roll (5)')
        cy.wait(1000)
        ...
        cy.task('queryDb', sqlFriedDumpling)
            .then((resultFriedDumpling) => {       
                ...
                cy.get('#quantityForCart1AND' + resultFriedDumpling[0].id).should('have.value', 1)    
                cy.get('#orderlist > form > div > div > div:nth-child(1) > img').should('have.attr', 'src','\\images\\' + resultFriedDumpling[0].gallery)
        ...
        const sqlEggRoll = 'select * from products where name = "Egg Roll (5)"'
            cy.task('queryDb', sqlEggRoll)
                .then((resultEggRoll) => {
                    priceEggRoll = resultEggRoll[0].price
    
                    cy.get('#quantityForCart2AND' + resultEggRoll[0].id).should('have.value', 1)
                    cy.get('#orderlist > form:nth-child(2) > div > div > div:nth-child(1) > img').should('have.attr', 'src','\\images\\' + resultEggRoll[0].gallery)
                    cy.get('#orderlist > form > div > div > div.col-md-6 > h5.pt-2').contains(resultEggRoll[0].name)
                    cy.get('#orderlist > form > div > div > div.col-md-6 > h5.pt-2').contains(resultEggRoll[0].description)
                    cy.get('#orderlist > form > div > div > div.col-md-6 > h5.pt-1').contains(resultEggRoll[0].price)
                })
        ...
        cy.task('queryDb', sqlRestaurant)
            .then((resultRestaurant) => {
                var taxRate = parseFloat(resultRestaurant[0].tax_rate)
                var price = priceFriedDumpling + priceEggRoll
                var tax = parseFloat((taxRate * price).toFixed(2))
                var total = parseFloat((price + tax).toFixed(2))
    ...
    it('Click appetizers, click plus button for fried dumpling, then click Empty Cart button, cancel to empty the cart', () => {
        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
                // Back to cart and check all values
    ...
    it('Click appetizers, click plus button for fried dumpling, then click Empty Cart button, ok to empty the cart', () => {
        const sql = 'select * from products where name = "Fried Dumpling (5)"'
        cy.task('queryDb', sql)
            .then((result) => {
                // Back to cart and check all values
    ...
    it('Click appetizers, click plus button for fried dumpling, then type "Not too spicy" for note and forgot to click Update Note button, then click Checkout button', () => {
    ...
    it('Click appetizers, click plus button for fried dumpling, then type "Not too spicy" for note and click Update Note button, then click Checkout button', () => {
        cy.get('#ordernote').type('Not too spicy')
