...
describe('Visit cart, add canned drink with coke, and check in the cart', () => {
    it('Click drinks, click plus button for canned drink with coke, then check all information on the screen', () => {
        cy.get('#cart_count').contains('1')

        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
                // Image
                const sqlDrinks = 'select * from drinks where name = "Canned Drink"'
    ...
    it('Click drinks, click plus button for canned drink with coke, then click plus button one more time', () => {
        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
    ...
    it('Click drinks, click plus button for canned drink with coke, then click plus button one time, then click minus button one time', () => {
        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
                // 1 is serialNumber in cart
    ...
    it('Click drinks, click plus button for canned drink with coke, then click Remove button', () => {
        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
                // cart count should be 1
    ...
    it('Click drinks, click plus button for canned drink with coke, then click Edit button -- change quantity to 3, 2, then click Update button', () => {
        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
                // cart count should be 1
    ...
    it('Click drinks, click plus button for canned drink with coke, then click Add More Items to same item', () => {
    ...
    it('Click drinks, click plus button for canned drink with coke, then click Add More Items to click plus button for Fountain soft drink with coke', () => {
        cy.get('.addMoreItems').click()
        cy.wait(1000)
        cy.clickDrinks()
        cy.wait(1000)
        ...
        cy.selectFountainDrinkSize('Medium')
        ...
        cy.task('queryDb', sqlCanned)
            .then((resultCanned) => {       
                priceCanned = parseFloat((resultCanned[0].price).toFixed(2))

                cy.get('#quantityForCart1AND' + resultCanned[0].id).should('have.value', 1)
                // Image
                const sqlCannedImg = 'select * from drinks where name = "Canned Drink"'
                cy.task('queryDb', sqlCannedImg)
                    .then((resultCannedImg) => {
                        cy.get('#orderlist > form > div > div > div:nth-child(1) > img').should('have.attr', 'src','\\images\\' + resultCannedImg[0].gallery)
                    })
                ...
                cy.get('#orderlist > form > div > div > div.col-md-6 > h5.pt-1').contains(priceCanned)
            })

        const sqlFountain = 'select * from products where name = "Fountain Soft Drink Medium"'
            cy.task('queryDb', sqlFountain)
                .then((resultFountain) => {
                    priceFountain = parseFloat((resultFountain[0].price).toFixed(2))
                    cy.get('#quantityForCart2AND' + resultFountain[0].id).should('have.value', 1)
                    // Image
                    const sqlFountainImg = 'select * from drinks where name = "Fountain Drink"'
                    cy.task('queryDb', sqlFountainImg)
                        .then((resultFountainImg) => {
                            cy.get('#orderlist > form:nth-child(2) > div > div > div:nth-child(1) > img').should('have.attr', 'src','\\images\\' + resultFountainImg[0].gallery)
                        })
        ...
        cy.task('queryDb', sqlRestaurant)
            .then((resultRestaurant) => {
                var taxRate = parseFloat(resultRestaurant[0].tax_rate)
                var price = parseFloat((priceCanned + priceFountain).toFixed(2))
                var tax = parseFloat((taxRate * price).toFixed(2))
                var total = parseFloat((price + tax).toFixed(2))
    ...
    it('Click drinks, click plus button for canned drink with coke, then click Empty Cart button, cancel to empty the cart', () => {
        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
                // Back to cart and check all values
    ...
    it('Click drinks, click plus button for canned drink with coke, then click Empty Cart button, ok to empty the cart', () => {
        const sql = 'select * from products where name = "Canned Soft Drink"'
        cy.task('queryDb', sql)
            .then((result) => {
                // Back to cart and check all values
    ...
    it('Click drinks, click plus button for canned drink with coke, then type "Not too spicy" for note and forgot to click Update Note button, then click Checkout button', () => {
    ...
    it('Click drinks, click plus button for canned drink with coke, then type "Not too spicy" for note and click Update Note button, then click Checkout button', () => {
        cy.get('#ordernote').type('No ice')
